...
def calculate_importances(nodes, n_features):
    all_importances = np.zeros((len(nodes), n_features))
    feature_gains = np.zeros(n_features)
    def _calculate_node_importances(node):
            samples = node.get("instance_count", 0)
            gain = node["gain"]
            feature = node["split_feature"]
            childs = node.get("children", None)
            feature_gains[feature] += gain * samples
            for child in childs:
                if "gain" in child:
                    _calculate_node_importances(child)
    for i, tree in enumerate(nodes):
        _calculate_node_importances(tree)
        all_importances[i] = feature_gains / feature_gains.sum()
        feature_gains = np.zeros(n_features)
...
class BorutaPy(BaseEstimator, TransformerMixin):
    ...
    def _get_imp(self, X, y):
        ...
        imp = calculate_importances(orjson.loads(self.estimator.get_json()), n_features=X.shape[1])
        return imp
