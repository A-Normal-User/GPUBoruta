...
from cuml.metrics import kl_divergence as cuml_KL
...
class BorutaPy(BaseEstimator, TransformerMixin):
    ...
    def _fit(self, X, y):
        ...
        self.imp_history = np.zeros(n_feat, dtype=float)
        sha_max_history = []
        ...
        while np.any(dec_reg == 0) and _iter < self.max_iter:
            ...
            self.imp_history = np.vstack((self.imp_history, cur_imp[0]))
        ...
        imp_history_rejected = self.imp_history[1:, not_selected] * -1
    ...
    def _get_KL_Divergence(self, A, B):
        A = cp.asarray(A)
        B = cp.asarray(B)
        epsilon = 1e-15
        A_clipped = cp.clip(A, epsilon, 1 - epsilon)
        B_clipped = cp.clip(B, epsilon, 1 - epsilon)
        return cp.sum(A_clipped * cp.log(A_clipped / B_clipped), axis=1).mean()

    def _get_imp_cuml(self, X, y):
        self.estimator.fit(X, y)
        FILmodel = self.estimator.convert_to_fil_model()
        
        X_gpu = cp.asarray(X)
        y_gpu = FILmodel.predict_proba(X_gpu)
        shape_X = X_gpu.shape
        
        imp = cp.zeros(shape_X[1])
        flush_index = cp.random.permutation(shape_X[0])
        
        # 只保留一个工作副本，减少内存分配
        X_work = X_gpu.copy()
        original_col = cp.zeros(shape_X[0])  # 临时存储原始列
        
        for i in range(shape_X[1]):
            # 保存原始列
            original_col[:] = X_work[:, i]
            # 就地修改
            X_work[:, i] = X_gpu[flush_index, i]
            
            # 预测
            y_pred = FILmodel.predict_proba(X_work)
            imp[i] = self._get_KL_Divergence(y_gpu, y_pred)
            
            # 恢复原始列
            X_work[:, i] = original_col
        return cp.asnumpy(imp)
    ...
    def _add_shadows_get_imps(self, X, y, dec_reg):
        ...
        imp = self._get_imp_cuml(np.hstack((x_cur, x_sha)), y)
