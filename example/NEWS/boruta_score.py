...
class BorutaPy(BaseEstimator, TransformerMixin):
    ...
    def _get_imp_memory_optimized(self, X, y):
        self.estimator.fit(X, y)
        FILmodel = self.estimator.convert_to_fil_model()
        
        X_gpu = cp.asarray(X)
        y_gpu = FILmodel.predict(X_gpu)
        shape_X = X_gpu.shape
        ...
        for i in range(shape_X[1]):
            ...
            # 预测
            y_pred = FILmodel.predict(X_work)
            imp[i] = cuml_mse(y_gpu, y_pred)
    ...
    def _add_shadows_get_imps(self, X, y, dec_reg):
        ...
        imp = self._get_imp_memory_optimized(np.hstack((x_cur, x_sha)), y)
